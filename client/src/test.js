...
test("GET /jersey color with name", async () => {
  for (var i = 0; i < 5; i++) {
    await supertest(app)
      .get("/jersey/color?name=Lauder")
      .expect(200)
      .then((response) => {
        expect(response.body.message).toBe("Hello, Lauder!");
        expect(response.body.jersey_color).not.toBe("white");
        expect(
          response.body.jersey_color == "red" ||
            response.body.jersey_color == "blue"
        ).toBeTruthy();
      });
  }
});
...
test("GET /search/matches basic 1", async () => {
  await supertest(app)
    .get("/search/matches?Home=Bay&Away=Hertha")
    .expect(200)
    .then((response) => {
      expect(response.body.results.length).toEqual(1);
...
test("GET /search/players basic 1", async () => {
  await supertest(app)
    .get("/search/players?Name=Pepe&Nationality=Spain&Club=Lazio")
    .expect(200)
    .then((response) => {
      expect(response.body.results.length).toEqual(1);
